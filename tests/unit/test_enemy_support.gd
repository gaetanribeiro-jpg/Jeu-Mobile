...
## vie : entamer quelqu'un était toujours un acquis. L'aumônier de la
...
## L'Archer, à huit, rend la main au joueur avant que l'aumônier ne joue.
...
func test_l_aumonier_annonce_un_soin_quand_un_allie_saigne() -> void:
	var board := _board()
	var chaplain := _enemy(board, &"iron_chaplain", Vector2i(8, 4), 1)
	var wounded := _enemy(board, &"iron_bulwark", Vector2i(7, 4), 2)
	...
	assert_true(intent.is_support(), "l'aumônier doit annoncer un soin")
	assert_eq(
...
func test_sans_blesse_il_ne_soigne_pas() -> void:
	var board := _board()
	var chaplain := _enemy(board, &"iron_chaplain", Vector2i(8, 4), 1)
	_enemy(board, &"iron_bulwark", Vector2i(7, 4), 2)
	_hero(board, &"warrior", Vector2i(1, 4), 3)
...
## borne l'aumônier est sa recharge, déclarée en données, pas une ruse.
func test_il_recoud_le_plus_blesse() -> void:
	var board := _board()
	var chaplain := _enemy(board, &"iron_chaplain", Vector2i(8, 4), 1)
	var scratched := _enemy(board, &"iron_bulwark", Vector2i(7, 4), 2)
	var mauled := _enemy(board, &"iron_pikeman", Vector2i(9, 4), 3)
	_hero(board, &"warrior", Vector2i(1, 4), 4)
	scratched.take_damage(10)
	mauled.take_damage(80)
...
func test_le_soin_s_annonce_avec_son_chiffre() -> void:
	...
	for entry: Dictionary in engine.telegraph():
		if int(entry["attacker_id"]) == chaplain.id:
			found = entry
	assert_false(found.is_empty(), "l'aumônier doit figurer au télégraphe")
...
func test_le_chiffre_annonce_est_plafonne_par_ce_qui_manque() -> void:
	var board := _board()
	_enemy(board, &"iron_chaplain", Vector2i(8, 4), 1)
	var wounded := _enemy(board, &"iron_bulwark", Vector2i(7, 4), 2)
	_hero(board, &"archer", Vector2i(1, 4), 3)
...
func test_le_soin_annonce_est_le_soin_applique() -> void:
	...
	# On fait tourner la timeline jusqu'à ce que l'aumônier ait exécuté.
	for _i in 40:
...
func test_un_ennemi_qui_a_bouge_n_annonce_pas_son_coup_immobile() -> void:
	var board := _board()
	var provost := _enemy(board, &"iron_provost", Vector2i(5, 4), 1)
	_hero(board, &"warrior", Vector2i(4, 4), 2)

	var ai := EnemyAI.new(CombatRng.new(1))
	assert_true(
		ai.intent_here(board, provost).is_attack(),
		"planté, il annonce son marteau"
	)
	provost.has_moved = true
	assert_false(
		ai.intent_here(board, provost).is_attack(),
		"ayant bougé, il n'a plus rien à annoncer"
	)
...
func test_le_telegraphe_rend_des_tableaux_de_meme_longueur() -> void:
	...
	_enemy(board, &"iron_chaplain", Vector2i(8, 4), 1)
	var wounded := _enemy(board, &"iron_bulwark", Vector2i(7, 4), 2)
	_enemy(board, &"iron_marksman", Vector2i(6, 4), 3)
